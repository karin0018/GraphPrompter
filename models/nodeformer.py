...
class NodeFormer(nn.Module):
    ...
    in_channels: node features dim
    hidden_channels: embed dim
    out_channels: label numbers
    ...
    def __init__(self, in_channels, hidden_channels, num_layers=2, num_heads=4, dropout=0.0,
                 ...
                 use_residual=True, use_act=False, use_jk=False, nb_gumbel_sample=10, rb_order=0, rb_trans='sigmoid', use_edge_loss=True): # out_channels
        ...
        if use_jk:
            self.fcs.append(nn.Linear(hidden_channels * num_layers + hidden_channels, hidden_channels))
        else:
    ...
    def forward(self, x, edge_index, query_mask, tau=1.0):
        '''
