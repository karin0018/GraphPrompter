...
class GraphDenosingSAGENet(MessagePassing):
    def __init__(self, x_dim,  emb_dim, edge_attr_dim, edge_fliter_feat_dim=1536, dropout = 0, aggr="add", transform_x=True, batch_norm=True):
        super(GraphDenosingSAGENet, self).__init__()
        # 去噪网络的层
        if edge_attr_dim is None:
            denosing_dim = x_dim*2
        else:
            denosing_dim = edge_attr_dim
        self.denoising_layers = torch.nn.Sequential(torch.nn.Linear(denosing_dim, 2*emb_dim), torch.nn.ReLU(),
    ...
    def forward(self, x, edge_index,  edge_attr=None, temperature=1,training=False, edge_fliter_feat=None):
        # assert edge_fliter_feat is not None
        if self.lin_edge_attr is None:
            edge_attr_x = torch.cat([x[edge_index[0]], x[edge_index[1]]], dim=1)
            edge_weight = self.denoising_layers(edge_attr_x)  # |E| x 1
            # pi = self.softmax(edge_weight)
            # sample = self.gumbel_softmax_sample(edge_weight, temperature, training)
            mask_values = self.sigmoid(edge_weight)
            # mask_values = self.softmax(sample)
        else:
            edge_attr_x = edge_attr
            edge_weight = self.denoising_layers(edge_attr_x) # |E| x 1
            # pi = self.softmax(edge_weight)
            mask_values = self.sigmoid(edge_weight)
        # mask_values = self.gumbel_softmax_sample(edge_weight, temperature, training)

        x_transformed = self.lin_x(x)

        if self.lin_edge_attr is None:
            x_msg = self.propagate(aggr=self.aggr, edge_index=edge_index, x=x_transformed, mask_values=mask_values)
        else:
            edge_attr_emb = self.lin_edge_attr(edge_attr)
            x_msg = self.propagate(aggr=self.aggr, edge_index=edge_index, x=x_transformed, edge_attr=edge_attr_emb, mask_values=mask_values)
    ...
    def message(self, x_j, edge_attr=None, mask_values=None):
        if self.lin_edge_attr is None:
            return x_j + (mask_values) 
        else:
            return (x_j + edge_attr) * (mask_values)
    ...
    #     r = self.sample_gumbel(logits.size()).to(logits.device)
    #     values = torch.log(logits) + r if istrain else torch.log(logits)
    #     values /= temperature
    #     y = self.softmax(values)
    #     return y
    ...
    def gumbel_softmax_sample(self, log_alpha, temperature, istrain):
        if istrain:
            ...
            gate_inputs = (gate_inputs.to(log_alpha.device) + log_alpha) / temperature
...
class SAGEConvSelfLoops(MessagePassing):
    ...
    def forward(self, x, edge_index,  edge_attr=None, temperature=None,training=False, edge_fliter_feat=None):
        x_transformed = self.lin_x(x)
        if self.lin_edge_attr is None:
            x_msg = self.propagate(aggr=self.aggr, edge_index=edge_index, x=x_transformed)
        else:
...
gnn_models = {
    ...
    "gat": GATv2Conv,
    "denosingSAGE": GraphDenosingSAGENet,
}
