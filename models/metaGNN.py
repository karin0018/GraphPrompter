...
class MetaGNNLayer(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr=None, start_right=None):
        ...
        # DEBUG
        # "IndexError: Encountered an index error. Please ensure that all indices in 'edge_index' point to valid indices in the interval [0, 19] (got interval [0, 47])
        # print('self.propagate ... ')
        # print(
        #     f'edge_index={edge_index.shape}, x={x.shape},kqv_x={kqv_x.shape}, edge_attr={edge_attr.shape}'
        # )
        # input()
...
class MetaGNN(torch.nn.Module, MetagraphLayer):
    ...
    def forward(self, x, edge_index, edge_attr, query_mask, start_right, **kwargs):
        ...
        edge_index_back = edge_index[:, query_in_mask].flip(0) # edge_index of query
