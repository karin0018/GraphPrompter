...
class BgGraphToSupernodePropagatorCat(torch.nn.Module, SupernodeAggrLayer):
    ...
    def forward(self,
        ...
        # graph_emb = torch.cat([all_node_emb[head_idxs] , all_node_emb[tail_idxs]], 1)
        return self.proj(graph_emb)


class BgGraphToSupernodePropagatorSAGPool(torch.nn.Module, SupernodeAggrLayer):
    ...
    def __init__(
        ...
        super().__init__()
        self.aggr = aggr
        self.attn_layer = torch.nn.Linear(emb_dim, 1)
        self.proj = torch.nn.Linear(2 * emb_dim, emb_dim)
        self.action = torch.nn.ReLU()
    ...
    def forward(self,
        ...
        :return:
        '''
        # assumed for node and edge
        attn = self.action(self.attn_layer(all_node_emb))
        x = torch.mul(all_node_emb,attn)
        graph_emb = scatter(src=x[supernode_edge_index[0]],
                            index=supernode_edge_index[1],
                            dim=0,
                            reduce=self.aggr)[supernode_idx, :]
        pool = global_max_pool(all_node_emb, graph_batch)
        graph_emb = torch.cat([graph_emb, pool], 1)
        return self.proj(graph_emb)
