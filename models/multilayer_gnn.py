...
class MultiLayerGNN(torch.nn.Module, BackgroundGNNLayer):
    ...
    def forward(self, x_orig, x, edge_index, edge_attr, supernode_edge_index=None, center_node_index = None, batch = None, temperature=1, training=False,edge_fliter_feat=None):
        '''
        ...
        for idx, module in enumerate(self.module_list):
            ...
            # print(f'reset_after_layer = {self.reset_after_layer}, idx = {idx}')
            ...
            x = module(x=x, edge_index=edge_index, edge_attr=edge_attr, temperature=temperature, training=training, edge_fliter_feat=edge_fliter_feat)
            x = self.act(x)
