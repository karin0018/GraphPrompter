...
def get_model(emb_dim=128, bert_dim=768, n_layer=1, input_dim=768, edge_attr_dim=4, classification_only=False,
              gnn_type="gin", add_to_dim_in=None, dropout = 0, reset_after_layer = None, batch_norm=True, edge_fliter_feat_dim=1536):
    ...
    elif gnn_type == "denosingSAGE":
        gnn1 = GNNConv(x_dim=input_dim + add_to_dim, edge_fliter_feat_dim=edge_fliter_feat_dim, edge_attr_dim=edge_attr_dim, emb_dim=emb_dim, dropout = dropout, aggr="mean", batch_norm=batch_norm)
    else:
        gnn1 = GNNConv(x_dim=input_dim + add_to_dim, edge_attr_dim=None, emb_dim=emb_dim, dropout = dropout, aggr="mean", batch_norm=batch_norm)
    ...
    for i in range(n_layer - 1):
        if gnn_type == "gat":
            ...
            all_gnns.append(GNNConv(x_dim=emb_dim, edge_fliter_feat_dim=edge_fliter_feat_dim, edge_attr_dim=edge_attr_dim, emb_dim=emb_dim, aggr="mean", batch_norm=batch_norm))
        else:
            all_gnns.append(GNNConv(x_dim=emb_dim, edge_attr_dim=None, emb_dim=emb_dim, aggr="mean", batch_norm=batch_norm))
