...
def arxiv_task(split, node_split="", split_labels=True, train_cap = 3, label_set = range(40), linear_probe=False, ogb_root="dataset",dataset_=None, edge_sampler=None, edge_sampler_type=None,eval=True,cache=False):
    ...
    my_task = MulticlassTask(label, label_set, train_label, linear_probe,dataset=dataset_,edge_sampler_type=edge_sampler_type,eval=eval,cache=cache) if edge_sampler else MulticlassTask(label, label_set, train_label, linear_probe,cache=cache)
...
def get_arxiv_dataloader(dataset, split, node_split, batch_size, n_way, n_shot, n_query, batch_count, root, bert, num_workers, aug, aug_test, split_labels, train_cap, linear_probe, label_set = range(40), **kwargs):
    ...
    cache = kwargs['cache']
    print(
        f'edge_sampler, edge_sampler_type,eval={edge_sampler, edge_sampler_type,eval}.... '
    )

    sampler = BatchSampler(
        batch_count,
        arxiv_task(split, node_split, split_labels, train_cap, label_set, linear_probe, root, dataset, edge_sampler, edge_sampler_type,eval,cache),
    ...
    if split == "train" or aug_test:
        ...
        aug = get_aug("")
    # NOTE change Collator to KGCollator
