...
class MulticlassTask(TaskBase):
    def __init__(self, labels, label_set, train_label=None, linear_probe=False,dataset=None, edge_sampler_type="degree",eval=False,cache=False):
        ...
        if train_label is not None:
            ...
            print(f'train_label is not None: self.train_label2idx={self.train_label2idx}' )

        # TODO modified the sampler type
        if dataset is not None and edge_sampler_type=='degree':
            # whole graph degree
            data = dataset.pyg_graph
            ...
            self.data = data
            self.edge_sampler_type = edge_sampler_type
            self.label2memidx = {label:None for label in label_set}
    ...
    def sample(self, num_label, num_member, num_shot, num_query, rng):
        if self.linear_probe:
            ...
            # NOTE not random sample labelset here, change to fix label
            # labels = rng.sample(self.label_set, num_label)
            labels = list(self.label_set)[:num_label] if self.cache else rng.sample(self.label_set, num_label)

        task = {}

        if self.train_label is None:

            # TODO Page rank sampler
            for label in labels:
                ...
                if self.data is not None:
                    if self.eval and self.edge_sampler_type == "degree":

                        if self.label2memidx[label] == None:
                            prob = (1. / self.deg_in[self.data.edge_index[0][members]]) + (
                                1. / self.deg_out[self.data.edge_index[1][members]])
                            rand = torch.rand(members.shape[0]).log() / (prob+ 1e-10)

                            if members.shape[0] < num_member:
                                self.label2memidx[label] = sample_func(range(members.shape[0]), k=num_member-1)
                            else:
                                self.label2memidx[label]  = rand.topk(num_member-1,dim=-1).indices

                        # select prompts
                        member_choices = members[self.label2memidx[label]].tolist()
                        if members.shape[0] >= num_member:
                            members = np.delete(members, self.label2memidx[label])
                        # select query
                        edge_idx = sample_func(range(len(members)),k=1)[0]
                        member_choices.append(members[edge_idx])
                        task[label] = member_choices
                    else:
                        prob = (1. / self.deg_in[self.data.edge_index[0][members]]) + (
                                1. / self.deg_out[self.data.edge_index[1][members]])
                        rand = torch.rand(members.shape[0]).log() / (prob+ 1e-10)
                        edge_sample = rand.topk(num_member, dim=-1).indices
                        task[label] = members[edge_sample].tolist()
