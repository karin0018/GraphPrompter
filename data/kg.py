...
def idx_split(n, fracs=[0.7, 0.1, 0.2]):
    # TODO set random seed defult 42
...
def kg_task_no_labels_split(labels, dataset, label_set, linear_probe, train_cap=3, split="train",edge_sampler=None,edge_sampler_type=None,eval=False,seed=42,cache=False):
    # labels = edge_attr
    ...
    # NOTE SHUFFLE LABELS_SET
    if cache:
        generator = random.Random(81)
        label_set = list(label_set)
        generator.shuffle(label_set)
    ...
    if split == "train":
        ...
        label = -1 - label
    # TODO add parameter for task: dataset=datset
    my_task = MulticlassTask(
        label,
        label_set,
        train_label,
        linear_probe=linear_probe,
        dataset=dataset, edge_sampler_type=edge_sampler_type,eval=eval,cache=cache) if edge_sampler else MulticlassTask(
            label,
            label_set,
            train_label,
            linear_probe=linear_probe,
            dataset=None,eval=eval,cache=cache)
    return my_task
...
def get_kg_dataloader(dataset, task_name, split, node_split, batch_size, n_way, n_shot, n_query, batch_count, root, num_workers, aug, aug_test, train_cap, linear_probe, label_set=None, all_test=False, **kwargs):
    ...
    seed = kwargs['seed']
    edge_sampler = kwargs['edge_sampler']
    edge_sampler_type = kwargs['edge_sampler_type']
    eval = kwargs['eval']
    cache = kwargs['cache']
    print(f'edge_sampler={edge_sampler, edge_sampler_type} all_test = {eval}.... ')
    ...
    if task_name == "same_graph":
        ...
        if split_labels:
            ...
            task = kg_task_no_labels_split(labels, dataset=dataset, train_cap=train_cap, split=split, label_set=label_set, linear_probe=linear_probe, edge_sampler=edge_sampler,edge_sampler_type=edge_sampler_type,eval=eval,seed=seed,cache=cache)
