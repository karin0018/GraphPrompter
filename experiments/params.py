...
def get_params():
    ...
    # adaptive select examples
    args.add_argument("-knn","--knn", action='store_true', help='use k-nearest neighbors')
    args.add_argument("-add_edges","--add_edges", action='store_true', help='use new task graph that add edges betweem each example and query node')
    args.add_argument("-cache","--cache", action='store_true', help='use cache')
    args.add_argument("-eval","--eval", action='store_true', help='evaluation sample')
    args.add_argument("-select","--select", action='store_true', help='pretrain select layers')
    args.add_argument("-text_sim","--text_sim", action='store_true', help='use text similarity in kNN')
    args.add_argument("-select_lambda", "--select_lambda", default=0.5, type=float,help='weights of pretrain select layers')
    args.add_argument("-contras_loss","--contras_loss", action='store_true', help='use contras_loss')
    args.add_argument("-edge_sampler", "--edge_sampler", action='store_true', help='use edge sampler instead random')
    args.add_argument("-edge_sampler_type","--edge_sampler_type", default=None, type=str)
    args.add_argument("-contras_weight", "--contras_weight", default=0.5, type=float)
    args.add_argument("-contras_temp", "--contras_temp", default=0.1, type=float)
    args.add_argument("-temp", "--temp", default=3, type=int, help='set the number of selected prompts from the candidate prompt set')
    
    args.add_argument("-add_norm","--add_norm", action='store_true', help='add normalization')
    args.add_argument("-use_edge_loss","--use_edge_loss", action='store_true', help='use edges loss to reg predict adj to original adj')
    args.add_argument(
        "-num_prototypes4label", "--num_prototypes4label", default=1, type=int
    )  # help='set the number of prototype for each label, defult is 1'
    args.add_argument(
        "-cache_cap", "--cache_cap", default=3, type=int,help='set the size of cache'
    ) 
    args.add_argument(
        "-temperature", "--temperature", default=1, type=int,help='temperature of denosing'
    ) 
