...
from experiments.loss import SupConLoss
...
class TrainerFS():
    def __init__(self, dataset, parameter):
        ...
        # TODO add new parameter to set whether using the adaptive prompts
        self.knn = self.parameter['knn']
        self.cache = self.parameter['cache']
        self.add_norm = self.parameter['add_norm']
        self.add_edges = self.parameter['add_edges']
        self.contras = self.parameter['contras_loss']
        self.contras_loss_func = SupConLoss(self.parameter['contras_temp'])
        self.use_edge_loss = self.parameter['use_edge_loss']
        self.temperature = self.parameter['temperature']

        initial_label_mlp = torch.nn.Linear(bert_dim, self.emb_dim)
        select_layers =  torch.nn.Sequential(
        ...
        if self.dataset_name in [
                "NELL", "ConceptNet", "FB15K-237", "Wiki", "WikiKG90M"
        ]:
        ...
        if self.dataset_name in ["CSG"]:
            edge_attr_dim = 128
        # if self.dataset_name in ["mag240m", "arxiv"]:
        #     edge_attr_dim = 1
            
        
        edge_fliter_feat_dim = None
        ...
        if self.parameter["layers"] != "SimpleDotProduct":
            ...
            layer_list = get_module_list(self.parameter["layers"], self.emb_dim, edge_attr_dim=edge_attr_dim,
                                         input_dim=self.parameter["input_dim"], dropout=self.parameter["dropout"], edge_fliter_feat_dim=edge_fliter_feat_dim,
                                         ...
                                         use_edge_loss=self.parameter['use_edge_loss'])

            layer_list = torch.nn.ModuleList(layer_list)
            self.model = SingleLayerGeneralGNN(layer_list=layer_list, initial_label_mlp=initial_label_mlp,  select_layers=select_layers,
                                            params=self.parameter, text_dropout=self.txt_dropout, num_prototypes4label=self.parameter['num_prototypes4label']) # initial_input_mlp = initial_input_mlp,
        else:
    ...
    def _build_dataloaders(self, dataset, dataset_name):
        ...
        # set seed
        kwargs['seed'] = self.parameter['seed']
        kwargs['eval'] = self.parameter['eval']
        kwargs['cache'] = self.parameter['cache']
        kwargs['select'] = self.parameter['select']
        kwargs['select_lambda'] = self.parameter['select_lambda']
        kwargs['text_sim'] = self.parameter['text_sim']
        kwargs['edge_sampler'] = self.parameter['edge_sampler']
        kwargs['edge_sampler_type'] = self.parameter['edge_sampler_type']
    ...
    def get_contrasive_loss(self,y_true_matrix,x_input):
        labels = y_true_matrix.nonzero()[:,1] # [bsz]
        loss = self.contras_loss_func(x_input,labels)
        return loss
    ...
    def train(self):
        ...
        with torch.no_grad():
            # self.model.eval()
            test_loss, test_acc, test_acc_std, test_aux_loss, test_contras_loss, test_link_loss,ranks = self.do_eval(self.test_dataloader)
        ...
        print(f'self.contras = {self.contras}')

        with torch.no_grad():
            # self.model.eval()
            val_loss, val_acc, val_acc_std, val_aux_loss, val_contras_loss, val_link_loss, ranks = self.do_eval(self.val_dataloader)
        ...
        for e in pbar:
            ...
            yt_o, yp_o, x_input, qry_idx, graph, link_loss_ = self.model(
                *batch,
            ...
            yt, yp = yt_o[qry_idx,:],yp_o[qry_idx,:]
            loss, acc = self.get_loss_and_acc(yt, yp) # get CE loss
            aux_loss = self.get_aux_loss(graph)
            weight = self.parameter["attr_regression_weight"]
            contras_weight = self.parameter["contras_weight"]
            contras_loss = self.get_contrasive_loss(yt_o,x_input)
            if self.use_edge_loss:
                loss -= 1 * sum(link_loss_) / len(link_loss_)
            if self.contras:
                total_loss = loss + contras_loss*contras_weight + aux_loss * weight 
            else:
            ...
            wandb.log({"train_loss": loss, "train_acc": acc, "train_aux_loss": aux_loss, "train_contrastive_loss": contras_loss, "train_total_loss": total_loss}, step=e)  # loss and acc here are both floats
            t_load += t2 - t1
            t_one_step += t3 - t2
            ...
            if e % self.eval_step == 0 and e != 0:
                ...
                with torch.no_grad():
                    self.model.eval()
                    val_loss, val_acc, val_acc_std, val_aux_loss, val_contras_loss, val_link_loss_global, ranks = self.do_eval(self.val_dataloader)
                ...
                pbar.write(f"Validation loss {val_loss} acc {val_acc} aux_loss {val_aux_loss} contrastive_loss {val_contras_loss}, link_loss_global {val_link_loss_global}")
                wandb.log({"valid_loss": val_loss, "valid_acc": val_acc, "valid_aux_loss": val_aux_loss,"valid_contrastive_loss": val_contras_loss,"val_link_loss_global":val_link_loss_global},
                          step=e)

                if self.train_val_dataloader is not None:
                    with torch.no_grad():
                        self.model.eval()
                        tval_loss, tval_acc, tval_acc_std, tval_aux_loss,tval_contras_loss, tval_link_loss_global, ranks = self.do_eval(self.train_val_dataloader)
                        wandb.log({"train_val_loss": tval_loss, "train_val_acc": tval_acc, "train_val_aux_loss": tval_aux_loss,"train_val_contrastive_loss": tval_contras_loss,"train_val_link_loss_global":tval_link_loss_global}, step=e)

                # Also evaluate on test set
                with torch.no_grad():
                    self.model.eval()
                    test_loss, test_acc, test_acc_std, test_aux_loss, test_contras_loss, test_link_loss_global, ranks = self.do_eval(self.test_dataloader)
                    log_dict = {"test_acc": test_acc, "test_loss": test_loss.cpu().detach().float(), "test_aux_loss": test_aux_loss,"test_contrastive_loss": test_contras_loss, "test_link_loss_global":test_link_loss_global, "test_acc_std": test_acc_std}
    ...
    def do_eval(self, dataloader, eff_len=None):
        ...
        all_contras_loss = []
        all_link_loss = []
        print(f'do eval, knn is {self.knn} ... cache is {self.cache}, add edges = {self.add_edges} add_norm is {self.add_norm}')
        for batch in tqdm(dataloader, leave=False):
            ...
            yt_o, yp_o, x_input, qry_idx, graph, link_loss_ = self.model(
                *batch,
                ...
                querys=self.querys,
                temperature=1.0,
                training=False,)  # apply the model
            yt, yp = yt_o[qry_idx,:],yp_o[qry_idx,:]
            ...
            all_contras_loss.append(contras_loss.item())
            if self.use_edge_loss:
                all_link_loss.append(sum(link_loss_)/len(link_loss_))
        ...
        contras_loss_global = sum(all_contras_loss) / len(all_contras_loss)
        link_loss_global = sum(all_link_loss) / len(all_link_loss) if self.use_edge_loss else 0
        torch.set_grad_enabled(True)
        if ranks is not None:
            ranks = {key: np.average([r[0][key] for r in ranks], weights=[r[1] for r in ranks]) for key in ranks[0][0]}
        return loss_global, acc_global, acc_batch_std, aux_loss_global, contras_loss_global,link_loss_global, ranks
